...
class LayersInfoCardDetails extends React.Component {
    ...
    switch(key) {
      case "fields": {
        if(args.data[key].hasOwnProperty("fieldArray")) {
          this._basicArrayOutput(list, key, args.data[key]["fieldArray"], uniqueId);
        } else {
          this._basicArrayOutput(list, key, args.data[key], uniqueId);
        }
        break;
      }
      case "indexes": {
        if(args.data[key].hasOwnProperty("indexArray")) {
          this._customArrayOutput(list, key, args.data[key]["indexArray"], uniqueId);
        } else {
          this._basicArrayOutput(list, key, args.data[key], uniqueId);
        }
        break;
      ...
      case "subtypes": {
        this._customArrayOutput(list, key, args.data[key], uniqueId);
        break;
      }
      ...
      case "spatialReference":
        this._basicStructOutput(list, key, args.data[key], uniqueId);
        break;
      default:
        this._basicOutput(list, key, args.data, uniqueId);
        break;
    }
  }
  ...
  _basicArrayOutput =(list, key, node, uniqueId) => {
    ...
    list.push(<div key={uniqueId}><Collapsible trigger={key} triggerOpenedClassName="collapse-open" triggerClassName ="collapse-close">
