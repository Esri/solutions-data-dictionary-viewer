...
class ServiceInfoCard extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      cardOptions: {
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <ServiceInfoCardDetails data={localData} id={localId} title={this.props.title} detailType={localType} />
      </Rnd>
    );
  }
...
class ServiceInfoCardDetails extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      subDetails: [],
        ...
        if(typeof(node[key]) === "object" && node[key] !== null) {
          ...
          if((JSON.stringify(node[key])).length > 50) {
            ...
            <Popover popoverClassName="bp3-popover-content-sizing" content={(JSON.stringify(node[key])).replace(/,/g, ", ")}>
              <div className="popup-link">{(JSON.stringify(node[key])).substring(0,50)  + "..."}</div>
            </Popover>
            </td>
            </tr>);
          } else {
            internal.push(<tr key={id}><td><div className="padding">{key}</div></td><td>{(JSON.stringify(node[key])).substring(0,50)}</td></tr>);
          }
        }
      }
    }
          ...
          if(node[i].hasOwnProperty(keyList[key])) {
            ...
            if(typeof(data[property]) === "object" && data[property] !== null) {
              if(Array.isArray(data[property])) {
                ...
                if(typeof(data[property][0]) === "object" && data[property] !== null) {
                  recursiveArray(internal, data[property], recordUnique, parent);
                } else {
                  if((JSON.stringify(data[property])).length > 50) {
                    ...
                    <Popover popoverClassName="bp3-popover-content-sizing" content={(JSON.stringify(data[property])).replace(/,/g, ", ")}>
                      <div className="popup-link">{(JSON.stringify(data[property])).substring(0,50)  + "..."}</div>
                    </Popover>
                    </td>);
                  } else {
                    rows.push(<td key={recordUnique}>{(JSON.stringify(data[property])).substring(0,50)}</td>);
                  }
                }
              } else {
  ...
  handleDialogOpen = (data, key) => {
    let status = this.state.dialogProps;
    status.isOpen = true;
    status.title = "world";
    status.data = "hello";
    this.setState({ dialogProps: status });
  }
  handleDialogClose = () => {
    let status = this.state.dialogProps;
    status.isOpen = false;
