...
export default class FieldCard extends React.Component <IProps, IState> {
  ...
  componentWillMount () {
    ...
    this.setState({ nodeData: cleanAlais }, () => {
      ...
      const filtered = this.props.domains.map((d: any) => {
    ...
    <div style={{ width: '100%', backgroundColor: '#fff', borderWidth: 2, borderStyle: 'solid', borderColor: '#000', float: 'left', display: 'inline-block' }}>
      ...
      />
      {
        ...
        <div style={{ paddingTop: 5, paddingBottom: 5, fontSize: 'smaller' }}>{this.buildCrumb()}<span style={{ fontWeight: 'bold' }}>Properties</span></div>
          ...
          {
            (this.state.fieldDescriptions?.[this.props.data.data.name])
          ...
          {(this.props.data.data?.precision) ? <div style={{ paddingTop: 5, paddingBottom: 5 }}><span style={{ fontWeight: 'bold' }}>Model Name:</span> {this.props.data.data.modelName}</div> : ''}
          {
            (this.props.data.data?.isNullable)
          ...
          <div style={{ paddingTop: 5, paddingBottom: 5 }}><span style={{ fontWeight: 'bold' }}>Required:</span> {(this.props.data.data?.required) ? (this.props.data.data.required) ? 'True' : 'False' : 'False'}</div>
          <div style={{ paddingTop: 5, paddingBottom: 5 }}><span style={{ fontWeight: 'bold' }}>Default Value:</span> {(this.props.data.data?.defaultValue) ? this.props.data.data.defaultValve : ''}</div>
          ...
          {(this.props.data.data?.precision) ? <div style={{ paddingTop: 5, paddingBottom: 5 }}><span style={{ fontWeight: 'bold' }}>Precision:</span> {this.props.data.data.precision}</div> : ''}
          {(this.props.data.data?.scale) ? <div style={{ paddingTop: 5, paddingBottom: 5 }}><span style={{ fontWeight: 'bold' }}>Scale:</span> {this.props.data.data.scale}</div> : ''}
          {
            (this.props.data.data?.domain)
  ...
  _createDomainExpand =(domain: any) => {
    ...
    if (domain?.codedValues) {
      domain.codedValues.forEach((d: any, z: number) => {
        vals.push(
          <tr key={z}>
  ...
  _createARExpand =() => {
    let arTable = null
    const vals = []
    this.state.ARList.forEach((ar: any, z: number) => {
      vals.push(
        <tr key={z}>
  ...
  _listAlias =(aliasList: any) => {
    ...
    if (aliasList.length > 1) {
      aliasList.forEach((v: any, i: number) => {
        list.push(
          <div key={i}>
  ...
  _fillExpandState =() => {
    ...
    let alias = null
    if (this.state.nodeData?.aliasName) {
  ...
  _checkARuse =(layerId: any) => {
    ...
    if (filteredDE.length > 0) {
      ...
      const ar = filteredDE[0].dataElement.attributeRules
      const usedAR = ar.filter((a: any) => {
