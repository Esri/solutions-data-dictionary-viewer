...
interface TerminalConfig {
  toTerminalConfig?: string
  toTerminalId?: string
  fromTerminalConfig?: string
  fromTerminalId?: string
  viaTerminalConfig?: string
  viaTerminalId?: string
}

interface RuleCodes extends TerminalConfig {
  fromassettypename?: string
  fromassetgroupname?: string
  toassettypename?: string
  toassetgroupname?: string
  viaassetgroupname?: string
  viaassettypename?: string
}
...
export default class TableCard extends React.Component <IProps, IState> {
  ...
  toggleExpandRuleType =(val: string) => {
    const newRuleState = this.state.expandRuleType
    if (newRuleState[val]) {
  ...
  _createRulesList = () => {
    ...
    const processRules = (rules: any, type: string) => {
      //console.log(rules);
      const rowList = []
      rules.forEach((fv: any, i: number) => {
        rowList.push(<tr key={fv.attributes.ruletypename + i}>
          ...
          { (type.includes('Junction')) ? <td style={{ fontSize: 'small' }}>{fv.attributes.fromterminalconfig}</td> : ''}
          { (type.includes('Junction')) ? <td style={{ fontSize: 'small' }}>{fv.attributes.fromterminalidname}</td> : ''}
          ...
          { (type.includes('Junction')) ? <td style={{ fontSize: 'small' }}>{fv.attributes.toterminalconfig}</td> : ''}
          { (type.includes('Junction')) ? <td style={{ fontSize: 'small' }}>{fv.attributes.toterminalidname}</td> : ''}
    ...
    if (Object.keys(this.state.rulesList).length > 0) {
      for (const key in this.state.rulesList) {
        const codeBlock = <div key={key} style={{ width: '100%' }}>
          ...
          <Table key={key}>
            ...
            <tr>
              ...
              { (key.includes('Junction')) ? <th style={{ fontSize: 'small', fontWeight: 'bold' }}>From Terminal Config</th> : ''}
              { (key.includes('Junction')) ? <th style={{ fontSize: 'small', fontWeight: 'bold' }}>From Terminal</th> : ''}
              <th style={{ fontSize: 'small', fontWeight: 'bold' }}>To Asset Group</th><th style={{ fontSize: 'small', fontWeight: 'bold' }}>To Asset Type</th>
              { (key.includes('Junction')) ? <th style={{ fontSize: 'small', fontWeight: 'bold' }}>To Terminal Config</th> : ''}
              { (key.includes('Junction')) ? <th style={{ fontSize: 'small', fontWeight: 'bold' }}>To Terminal</th> : ''}
  ...
  _createCategoriesList = () => {
    ...
    if (this.props.data.data.categories.length > 0) {
      this.props.data.data.categories.forEach((c: string, i: number) => {
        arrList.push(
          <tr key={+i}>
  ...
  _requestRules = async () => {
    ...
    const checkTables = this.props.data.crumb.some((c: any) => {
      return c.type === 'Table'
    })
    if (checkTables) {
    ...
    //end table check
    const rulesTable = this.props.dataElements.filter((de: any) => {
      return de.dataElement.aliasName === 'Rules'
    })
    this.props.dataElements.forEach((de: any) => {
      if (de.dataElement?.domainNetworks) {
        unde = de.dataElement
        de.dataElement.domainNetworks.forEach((dn: any) => {
    ...
    if (rulesTable.length > 0) {
      if (this.props.config.useCache) {
        ...
        if (typeof this.props.cacheData.connectivityRules[sourceId[0].sourceId] !== 'undefined') {
          ...
          rulesList.features = []
          allRulesForSource.forEach((ar: any) => {
            const filteredRules = ar.features.filter((f: any) => {
              return (
  ...
  _processRules =(data: any, sources: any, unde: any, sourceId: any) => {
    const rulesList = []
    const distinctRuleType = []
    data.features.forEach((feat: any) => {
      const ruleDesc = this._matchRuleType(feat.attributes.ruletype)
      if (typeof rulesList[ruleDesc] === 'undefined') {
        rulesList[ruleDesc] = []
        distinctRuleType.push(ruleDesc)
      }
      const toSource = sources.filter((s: any) => {
        return s.sourceId === feat.attributes.tonetworksourceid
      })
      const fromSource = sources.filter((s: any) => {
        return s.sourceId === feat.attributes.fromnetworksourceid
      })
      const viaSource = sources.filter((s: any) => {
        return s.sourceId === feat.attributes.vianetworksourceid
      })
      const descriptions: RuleCodes = this._matchCodes(feat.attributes, toSource, fromSource, viaSource, unde)
  ...
  _matchCodes =(data: any, toSource: any, fromSource: any, viaSource: any, de: any) => {
    let returnData = { fromassettypename: null, fromassetgroupname: null, toassettypename: null, toassetgroupname: null, viaassetgroupname: null, viaassettypename: null }
    if (toSource.length > 0) {
      toSource[0].assetGroups.forEach((ag: any) => {
        if (ag.assetGroupCode === data.toassetgroup) {
          returnData.toassetgroupname = ag.assetGroupName
          ag.assetTypes.forEach((at: any) => {
            if (at.assetTypeCode === data.toassettype) {
              returnData.toassettypename = at.assetTypeName
              const termConf = this._matchTerminalConfig(at, de, data, 'to')
    ...
    if (fromSource.length > 0) {
      fromSource[0].assetGroups.forEach((ag: any) => {
        if (ag.assetGroupCode === data.fromassetgroup) {
          ...
          ag.assetTypes.forEach((at: any) => {
            if (at.assetTypeCode === data.fromassettype) {
              ...
              const termConf = this._matchTerminalConfig(at, de, data, 'from')
              returnData = { ...returnData, ...termConf }
            }
          })
        }
    ...
    if (viaSource.length > 0) {
      viaSource[0].assetGroups.forEach((ag: any) => {
        if (ag.assetGroupCode === data.viaassetgroup) {
          ...
          ag.assetTypes.forEach((at: any) => {
            if (at.assetTypeCode === data.viaassettype) {
              ...
              const termConf = this._matchTerminalConfig(at, de, data, 'via')
              returnData = { ...returnData, ...termConf }
            }
          })
        }
  ...
  _matchTerminalConfig =(at: any, sourceLayer: any, data: any, direction: string) => {
    let terminalConfig: TerminalConfig = null
    const terminal = sourceLayer.terminalConfigurations.filter((tc: any) => {
      return tc.terminalConfigurationId === at.terminalConfigurationId
    })
    if (terminal.length > 0) {
      terminal[0].terminals.forEach((t: any) => {
        if (direction === 'to') {
  ...
  _lookupTC =(lookup: any) => {
    ...
    this.props.controllerDS.dataElement.terminalConfigurations.forEach((tc: any) => {
  ...
  _lookupLayerByAlias =() => {
    ...
    const parentLayer = this.props.dataElements.filter((de: any) => {
      return (de.layerId === this.props.data.parentId)
    })
    this.props.data.crumb.forEach((c: any, i: number) => {
      if (c.type === 'Subtype') {
        parentLayer[0].dataElement.subtypes.forEach((st: any) => {
                            ...
                            for (let e = 0; e < edomLevel.length; e++) {
                              ...
                              if (edomvLevel.length > 0) {
                                if (parseInt(edomvLevel[0].innerHTML) === parseInt(this.state.nodeData.assetTypeCode)) {
                                  ...
                                  const edomvdaltLevel = edomLevel[e].getElementsByTagName('edomvdalt')
