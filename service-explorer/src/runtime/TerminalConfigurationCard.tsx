...
export default class TerminalConfigurationCard extends React.Component <IProps, IState> {
  ...
  _createTerminalTable =() => {
    const arrList = []
    this.state.nodeData.terminals.forEach((t: any, i: number) => {
      arrList.push(
        <tr key={i}>
  ...
  _createTerminalPathsTable =() => {
    ...
    this.state.nodeData.validConfigurationPaths.forEach((vp: any, i: number) => {
      arrPath = []
      vp.terminalPaths.forEach((tp: any, z: number) => {
        arrPath.push(
          <tr key={z}>
