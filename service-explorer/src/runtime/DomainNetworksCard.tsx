...
export default class DomainNetworksCard extends React.Component <IProps, IState> {
          ...
          <div style={{ width: '100%', paddingLeft: 10, paddingRight: 10, wordWrap: 'break-word', whiteSpace: 'normal' }}>
            ...
            <Table hover>
                  ...
                  <tr>
                    ...
                    <th style={{ fontSize: 'small', fontWeight: 'bold' }}>Tiers</th>
                  </tr>
                  </thead>
                  <tbody>
                    {this._createList()}
  ...
  _createList = () => {
    const arrList = []
    this.props.data.data.forEach((ar: any, i: number) => {
      arrList.push(
          <tr key={i}>
            <td style={{ fontSize: 'small' }}>
            <div onClick={() => { this.props.callbackLinkage(ar.domainNetworkName, 'Domain Network', this.props.panel) }} style={{ display: 'inline-block', verticalAlign: 'top', paddingRight: 5, cursor: 'pointer' }}><Icon icon={linkIcon} size='12' color='#333' /> {ar.domainNetworkName} </div>
            </td>
            <td style={{ fontSize: 'small' }}>{ar.tiers.length}</td>
          </tr>
      )
    })
    return arrList
