...
export default class IndexCard extends React.Component <IProps, IState> {
  ...
  componentWillMount () {
    const fieldList = {}
    let fields = []
    if (this.props.data.data.fields?.fieldArray) {
      this.props.data.data.fields.fieldArray.forEach((fd: any) => {
    ...
    <div style={{ width: '100%', backgroundColor: '#fff', borderWidth: 2, borderStyle: 'solid', borderColor: '#000', float: 'left', display: 'inline-block' }}>
      ...
      />
      {
        ...
        <div style={{ width: '100%', paddingLeft: 10, paddingRight: 10, wordWrap: 'break-word', whiteSpace: 'normal' }}>
          ...
          <div style={{ paddingTop: 5, paddingBottom: 5 }}><span style={{ fontWeight: 'bold' }}>Ascending:</span> {(this.props.data.data?.isAscending) ? (this.props.data.data.isAscending) ? 'True' : 'False' : 'False'}</div>
          <div style={{ paddingTop: 5, paddingBottom: 5 }}><span style={{ fontWeight: 'bold' }}>Unique:</span> {(this.props.data.data?.isUnique) ? (this.props.data.data.isUnique) ? 'True' : 'False' : 'False'}</div>
  ...
  _createFieldList =() => {
    const arrList = []
    const usedFields = []
    this.state.fields.forEach((fi: any, z: number) => {
      if (!usedFields.includes(fi.name)) {
        let fieldDetailsTable = null
        if (fi.constructor === Object) {
          fieldDetailsTable = this._createFieldsExpand(fi)
        }
        const alias = (fi?.aliasName) ? fi.aliasName : fi
        const fieldName = <span><div style={{ textAlign: 'left' }}>{(fieldDetailsTable !== null) ? (this.state.fieldHolder[fi.name]) ? <Icon icon={downArrowIcon} size='12' color='#333' /> : <Icon icon={rightArrowIcon} size='12' color='#333' /> : ''} {fi?.name ? fi.name : fi}</div></span>
