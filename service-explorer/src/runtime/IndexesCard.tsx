...
export default class IndexesCard extends React.Component <IProps, IState> {
        ...
        <div style={{ paddingTop: 5, paddingBottom: 5, fontSize: 'smaller' }}>{this.buildCrumb()}<span style={{ fontWeight: 'bold' }}>{this.props.data.type}</span></div>
          ...
          <Table hover>
                ...
                <tr>
                  ...
                  <th style={{ fontSize: 'small', fontWeight: 'bold' }}>Unique</th>
                  <th style={{ fontSize: 'small', fontWeight: 'bold' }}>Ascending</th>
                  <th style={{ fontSize: 'small', fontWeight: 'bold' }}>Fields</th>
                </tr>
                </thead>
                <tbody>
                  {this._createFList()}
  ...
  _createFList = () => {
    const arrList = []
    this.props.data.data.forEach((f: any, i: number) => {
      const fieldList = []
      let arrFields
      if (f.fields?.fieldArray) {
        arrFields = f.fields.fieldArray
      } else {
        arrFields = f.fields.split(',')
      }
      arrFields.forEach((a: any, z: number) => {
        const value = (a?.name) ? a.name : a
        fieldList.push(
