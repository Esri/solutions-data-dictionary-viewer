...
export default class CategoryCard extends React.Component <IProps, IState> {
  ...
  componentWillMount () {
    const atCopy = { ...this.state.expandAT }
    const dn = this._findUN()
    if (dn !== null) {
      dn.forEach((d: any, i: number) => {
        d.junctionSources.forEach((js: any) => {
          js.assetGroups.forEach((ag: any) => {
  ...
  _createActiveTable =() => {
    ...
    if (dn !== null) {
      dn.forEach((d: any, i: number) => {
        d.junctionSources.forEach((js: any) => {
          js.assetGroups.forEach((ag: any) => {
            const atTable = this._createAssetTypeTable(ag.assetTypes, ag.assetGroupName, this._layerLookup(js.layerId))
            if (atTable !== null) {
              arrList.push(
                <tr key={i}>
  ...
  _createAssetTypeTable =(atList: any, ag: string, layerName: string) => {
    const arrList = []
    let tableObj = null
    atList.forEach((at: any, i: number) => {
