...
export default class SubtypeCard extends React.Component <IProps, IState> {
  ...
  componentWillMount() {
    ...
    let domainList = {};
    let fieldList = {};
    this.state.nodeData.fieldInfos.map((fi: any, i: number)=>{
      domainList[fi.domainName] = false;
    });
    this.props.data.fields.map((fd: any) => {
      fieldList[fd.name] = false;
    });
    this.setState({domainHolder: domainList, fieldHolder:fieldList});
  }
    ...
    <div style={{width: this.state.width, backgroundColor: "#fff", borderWidth:2, borderStyle:"solid", borderColor:"#000", float:"left", display:"inline-block"}}>
      <Navbar color="dark" expand="md">
        ...
        <Nav className="ml-auto"  tabs>
          ...
          <NavItem>
            <NavLink onClick={() => { this.toggleTabs('Statistics'); }}><Badge color={this.state.statsBadge}>Statistics</Badge></NavLink>
          </NavItem>
          <NavItem>
            <NavLink onClick={() => { this.toggleTabs('Diagrams'); }}><Badge color={this.state.diagramsBadge}>(WIP)</Badge></NavLink>
          </NavItem>
          <NavItem>
            <NavLink>
      ...
      <TabContent activeTab={this.state.activeTab}>
        ...
        <div style={{width: this.state.width, paddingLeft:10, paddingRight:10}}>
          ...
          <div style={{paddingTop:5, paddingBottom:5}}>Name: {this.state.nodeData.subtypeName}</div>
          <div style={{paddingTop:5, paddingBottom:5}}>Code: {this.state.nodeData.subtypeCode}</div>
          <div style={{color:"blue", paddingTop:5, paddingBottom:5}} onClick={()=>{this.toggleExpandFieldBlock();}}>Fields:</div>
          ...
          <div style={{color:"blue", paddingTop:5, paddingBottom:5}} onClick={()=>{this.toggleExpandAR()}}>Participating Attributes Rules:</div>
          ...
          <div style={{color:"blue", paddingTop:5, paddingBottom:5}} onClick={()=>{this.toggleExpandCAV()}}>Contingent Attribute Values:</div>
        ...
        <TabPane tabId="Diagrams">
          <div style={{width: this.state.width, paddingLeft:10, paddingRight:10}}>
            ...
            <div style={{width: this.state.width, overflowX:"auto"}}>
  ...
  _createFieldList = () => {
    ...
    this.state.nodeData.fieldInfos.map((fi: any, i: number)=>{
      ...
      if(fieldAlias.length > 0) {
        ...
        fieldName = <span><div style={{textAlign: "left"}} dangerouslySetInnerHTML={{__html: alias}}></div><div style={{textAlign: "left"}}>{"("+fi.fieldName+")"}</div></span>;
      }
      if(domain.length > 0) {
      ...
      arrList.push(<tr key={i}>
        ...
        <div style={{fontSize:"small", color:"blue"}} onClick={()=>{
        ...
        <td style={{fontSize:"small"}}>
          <div color="link" style={{fontSize:"small", color:"blue"}} onClick={()=>{
    ...
    this.props.data.fields.map((fi: any, z: number)=>{
      if(usedFields.indexOf(fi.name) === -1 && fi.name !== "shape") {
        let fieldDetailsTable = this._createFieldsExpand(fi.name);
        let alias = fi.aliasName;
        if(alias.indexOf(":") > -1) {
          //alias = alias.substring(0,alias.indexOf(":"));
          alias = this._handleAliasBrackets(alias, fi.name);
        }
        let fieldName = <span><div style={{textAlign: "left"}} dangerouslySetInnerHTML={{__html: alias}}></div><div style={{textAlign: "left"}}>{"("+fi.name+")"}</div></span>;
